...
const SelectServicesStep: React.FC<SelectServicesStepProps> = ({ availableServices, onNext, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div className="p-4 flex-shrink-0 bg-white z-10 border-b-4" style={{ borderColor: '#0B3559' }}>
        <h1 className="text-2xl font-black text-center tracking-tighter mb-4" style={{ color: '#0B3559' }}>Service Selection</h1>
        <div className="relative">
            <input 
                ...
                className="w-full p-4 border-4 rounded-2xl font-black shadow-inner outline-none focus:bg-white focus:border-[#5890A6] transition-all"
                style={{ backgroundColor: '#F0F4F8', borderColor: '#E2EAF0', color: '#0B3559' }}
            />
        </div>
        <div className="flex space-x-2 mt-4 overflow-x-auto pb-2 scrollbar-hide">
            {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 text-xs font-black rounded-full whitespace-nowrap transition-all uppercase tracking-widest ${activeCategory === cat || (cat === 'All' && !activeCategory) ? 'bg-brand-accent text-white shadow-lg scale-105' : 'border'}`} style={activeCategory === cat || (cat === 'All' && !activeCategory) ? {} : { backgroundColor: '#F0F4F8', color: '#8EB1BF', borderColor: '#C5D5DE' }}>
                    {cat}
                </button>
            ))}
        </div>
      ...
      <div className="flex-grow overflow-y-auto px-4 pb-48 pt-4">
        ...
        {Object.entries(servicesByCategory).map(([category, services]) => (
            <div key={category} className="mb-8">
                <h2 className="font-black text-xs uppercase tracking-widest mb-4 flex items-center" style={{ color: '#8EB1BF' }}>
                    <span className="flex-grow h-px mr-3" style={{ backgroundColor: '#E2EAF0' }}></span>
                    {category}
                    <span className="flex-grow h-px ml-3" style={{ backgroundColor: '#E2EAF0' }}></span>
                </h2>
                    ...
                    {(services as Service[]).map((service: Service) => {
                        ...
                        return (
                            <button 
                                ...
                                className={`p-5 rounded-[32px] shadow-sm transition-all duration-300 text-left flex flex-col justify-between h-36 border-4 relative overflow-hidden ${ isSelected ? 'bg-brand-accent text-white border-black/20 scale-95 shadow-inner' : 'bg-white border-[#F0F4F8]'}`}
                                style={!isSelected ? { color: '#0B3559' } : {}}
                            >
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-5 bg-white border-t-8 z-40 pb-10" style={{ borderColor: '#0B3559', boxShadow: '0 -20px 50px rgba(11, 53, 89, 0.2)' }}>
        <button 
            onClick={handleNext} 
            disabled={selectedIds.size === 0} 
            className="w-full bg-brand-primary text-white font-black py-5 px-4 rounded-[32px] shadow-2xl active:scale-95 transition-all disabled:opacity-40 disabled:shadow-none mb-4 text-xl border-b-8 border-black/20"
        >
          CONFIRM ({selectedIds.size})
        </button>
        <button onClick={onBack} className="w-full font-black py-2 uppercase text-xs tracking-[0.2em] opacity-40 hover:opacity-100 transition-opacity" style={{ color: '#0B3559' }}>
          Return to Client
        </button>
      </div>
    </div>
